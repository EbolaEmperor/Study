//----------------------------------------------------------
// This code is for inject turbulence test
// 
                                  ...
                                  const unsigned int component) const
{
  ...
  return max_velocity * 
         4 * p[1] * (0.4-p[1]) / (0.4 * 0.4) 
         * (1.0 - exp(-10. * this->get_time()));
}
                                  ...
                                  const unsigned int component) const
{
  ...
  return max_velocity * 
         4 * p[1] * (0.4-p[1]) / (0.4 * 0.4) 
         * 10. * exp(-10. * this->get_time());
}


template <int dim>
class OutflowBoundaryTerm : public Function<dim>
{
public:
  virtual double value(const Point<dim>  &p,
                        const unsigned int component = 0) const override;
...
template <int dim>
double OutflowBoundaryTerm<dim>::value(const Point<dim> & p,
                                  const unsigned int component) const
{
  (void)component;
  Assert(component == 0, ExcIndexRange(component, 0, 1));
...
template <int dim>
class OutflowBoundaryTermDt : public Function<dim>
{
public:
  virtual double value(const Point<dim>  &p,
                        const unsigned int component = 0) const override;
...
template <int dim>
double OutflowBoundaryTermDt<dim>::value(const Point<dim> & p,
                                  const unsigned int component) const
{
  (void)component;
  Assert(component == 0, ExcIndexRange(component, 0, 1));
...
INSE<dim>::INSE
  ...
  , level(N)
  , end_time(T)
  , time_step(2e-3/(1<<level))
{}


template <int dim>
void INSE<dim>::make_mesh(){
  Triangulation<dim> tria1, tria2, tria3, tria4, tria5, tria6, tria7, tria8;
  ...
  GridGenerator::subdivided_hyper_rectangle(
    tria4,
    std::vector<unsigned int>({23U, 2U}),
    Point<2>(0.08, 0.),
    Point<2>(2., 0.16),
    false);
  GridGenerator::subdivided_hyper_rectangle(
  ...
  GridGenerator::subdivided_hyper_rectangle(
    tria7,
    std::vector<unsigned int>({6U, 10U}),
    Point<2>(-.4, 0.),
    Point<2>(-.16, .4),
    false);
  GridGenerator::subdivided_hyper_rectangle(
    tria8,
    std::vector<unsigned int>({4U, 2U}),
    Point<2>(-.16, .16),
    Point<2>(0., .24),
    false);
  GridGenerator::merge_triangulations(
    {&tria7, &tria8, &tria1, &tria2, &tria3, &tria4, &tria5, &tria6},
    triangulation);
       ...
       ++cell)
  {
    for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
      ...
      {
        if (std::abs(cell->face(f)->center()[0] - 2.) < 1e-10)
          cell->face(f)->set_all_boundary_ids(4);
        else if (std::abs(cell->face(f)->center()[0] + .4) < 1e-10)
...
void INSE<dim>::make_constraints_u1(const double &t)
{
  ...
  VectorTools::interpolate_boundary_values(dof_handler,
                                           ...
                                           Functions::ZeroFunction<dim>(),
                                           constraints_u1);
  InflowBoundaryTerm<dim> in_boundary;
  in_boundary.set_time(t);
  VectorTools::interpolate_boundary_values(dof_handler,
                                           2,
                                           in_boundary,
                                           constraints_u1);
  OutflowBoundaryTerm<dim> out_boundary;
  out_boundary.set_time(t);
  VectorTools::interpolate_boundary_values(dof_handler,
                                           4,
                                           out_boundary,
                                           constraints_u1);
...
for(int th = 0; th < n_thr; th++)
{
  ...
  InflowBoundaryTermDt<dim> in_boundary_dt;
  in_boundary_dt.set_time(time);
  OutflowBoundaryTermDt<dim> out_boundary_dt;
  out_boundary_dt.set_time(time);
  int cnt = 0;
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (const auto &f : cell->face_iterators()){
        ...
        for (const unsigned int q_index : fe_face_values.quadrature_point_indices())
        {
          ...
          double gdt = 0;
          if(f->boundary_id() == 2) gdt = in_boundary_dt.value(q_face_points[q_index]);
          if(f->boundary_id() == 4) gdt = out_boundary_dt.value(q_face_points[q_index]);
...
template <int dim>
void INSE<dim>::run(){
  time_step /= max_velocity/1.5;
  if(max_velocity > 7.4999) time_step /= 2;
...
int main(int argc, const char *argv[]){
  ...
  if(argc < 3){
    ...
    std::cerr << "  ./inject-turbulence N T [Um]" << std::endl;
    std::cerr << "where N is the refinment times, T is end_time, Um is the scale of max-velocity." << std::endl;
    std::cerr << "  [default: Um=1.0, max-velocity=1.5Um, proposed: N=4 and T=2.0]" << std::endl;
    return -1;
  }
