...
namespace Step35
{
  ...
  {
  SphericalManifold<2> boundary(Point<2>(5, 2));
  Triangulation<2> left, middle, right, tmp, tmp2;

  GridGenerator::subdivided_hyper_rectangle(
      right,
      std::vector<unsigned int>({72U, 16U}),
      Point<2>(7, 0),
      Point<2>(25, 4.1),
      false);

  GridGenerator::hyper_shell(middle, Point<2>(5, 2), 0.5, 2, 4, true);
  ...
  for (Triangulation<2>::cell_iterator cell = middle.begin();
    ...
    for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
    {
      ...
      for (unsigned int v = 0; v < GeometryInfo<2>::vertices_per_face; ++v)
      {
        ...
        if (std::abs(vertex.distance(Point<2>(5, 2)) - 0.5) > 1e-8)
        {
          is_inner_rim = false;
          break;
        }
  ...
  for (Triangulation<2>::cell_iterator cell = middle.begin();
    ...
    for (unsigned int v = 0; v < GeometryInfo<2>::vertices_per_cell; ++v)
    {
      ...
      if (std::abs(vertex[0] - 7) < 1e-10 &&
          std::abs(vertex[1] - 2) < 1e-10)
        vertex = Point<2>(7, 2.05);
      else if (std::abs(vertex[0] - 6) < 1e-10 &&
               std::abs(vertex[1] - 3) < 1e-10)
        vertex = Point<2>(7, 4.1);
      else if (std::abs(vertex[0] - 6) < 1e-10 &&
               std::abs(vertex[1] - 1) < 1e-10)
        vertex = Point<2>(7, 0);
      else if (std::abs(vertex[0] - 5) < 1e-10 &&
               std::abs(vertex[1] - 4) < 1e-10)
        vertex = Point<2>(5, 4.1);
      else if (std::abs(vertex[0] - 5) < 1e-10 &&
               std::abs(vertex[1] - 0.0) < 1e-10)
        vertex = Point<2>(5, 0.0);
      else if (std::abs(vertex[0] - 4) < 1e-10 &&
               std::abs(vertex[1] - 3) < 1e-10)
        vertex = Point<2>(3, 4.1);
      else if (std::abs(vertex[0] - 4) < 1e-10 &&
               std::abs(vertex[1] - 1) < 1e-10)
        vertex = Point<2>(3, 0);
      else if (std::abs(vertex[0] - 3) < 1e-10 &&
               std::abs(vertex[1] - 2) < 1e-10)
        vertex = Point<2>(3, 2.05);
      else if (std::abs(vertex[0] - 5.7) < 0.1 &&
               std::abs(vertex[1] - 1.3) < 0.1)
        vertex = Point<2>(6.177, 0.823), std::cerr << "yes" << std::endl;
      else if (std::abs(vertex[0] - 5.7) < 0.1 &&
               std::abs(vertex[1] - 2.7) < 0.1)
        vertex = Point<2>(6.177, 3.227);
      else if (std::abs(vertex[0] - 4.3) < 0.1 &&
               std::abs(vertex[1] - 1.3) < 0.1)
        vertex = Point<2>(3.823, 0.823);
      else if (std::abs(vertex[0] - 4.3) < 0.1 &&
               std::abs(vertex[1] - 2.7) < 0.1)
        vertex = Point<2>(3.823, 3.227);
    }
  middle.refine_global(3);
       ...
       ++cell)
  {
    for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
    {
      ...
      {
        if (std::abs(cell->face(f)->center()[0] - 25) < 1e-12)
        {
          cell->face(f)->set_all_boundary_ids(3);
        }
        else if (std::abs(cell->face(f)->center()[0] - 3) < 1e-12)
        {
          cell->face(f)->set_all_boundary_ids(2);
        }
        else if (std::abs(cell->face(f)->center()[1] - 4.1) < 1e-12)
        {
  ...
  void NavierStokesProjection<dim>::output_results(const unsigned int step)
  {
    ...
    std::ofstream output("solution/solution-" + Utilities::int_to_string(step, 5) +
                         ".vtk");
