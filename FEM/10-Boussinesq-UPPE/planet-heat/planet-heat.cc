...
{
  forcing_1.reinit(solution_u1.size());
  forcing_2.reinit(solution_u1.size());
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (const unsigned int i : fe_values.dof_indices())
      {
        forcing_1[local_dof_indices[i]] += cell_rhs_1(i);
        forcing_2[local_dof_indices[i]] += cell_rhs_2(i);
      }
    }
}
...
void Boussinesq<dim>::run(){
  ...
  while(time <= end_time){
    ...
    system_rhs.add(0.5*time_step, forcing_1);
    ...
    system_rhs.add(0.5*time_step, forcing_2);
