...
struct Vertex
{
  ...
  bool operator < (const Vertex &rhs) const
  {
    return dcmp(pos[0], rhs.pos[0])==-1
        ...
        || ( dcmp(pos[0], rhs.pos[0])==0 && dcmp(pos[1], rhs.pos[1])==0
          && dcmp(pos[2], rhs.pos[2])==-1);
  }
};
...
private:
  ...
  std::map<types::global_dof_index, Point<3>> dof_location_map;
  ...
  unsigned corse2fine(const unsigned &i) const{
    return 2*(1+2*pow2level)*(1+2*pow2level)*(i/((1+pow2level)*(1+pow2level))) 
           + 2*(1+2*pow2level)*(i/(1+pow2level)%(1+pow2level))
           + 2*(i%(1+pow2level));
  }
};
...
void HiercharchicalPreconditioner::initialize()
{
  ...
  dof_location_map = DoFTools::map_dofs_to_support_points(MappingFE<3>(fe), dof_handler);
  ...
  vmult(Vector<double> &dst, const Vector<double> &src) const
{
  ...
  // full-weighting
  const unsigned pw = (pow2level<<1) + 1;
  const unsigned pw2 = pw * pw;
  ...
  for(unsigned i = 0; i < system_rhs.size(); i++, it++){
    ...
    for(int dx = -1; dx <= 1; dx++){
      if( (dx==-1 && pos[0]<1e-8) || (dx==1 && pos[0]>1-1e-8) ) continue;
      for(int dy = -1; dy <= 1; dy++){
        if( (dy==-1 && pos[1]<1e-8) || (dy==1 && pos[1]>1-1e-8) ) continue;
        for(int dz = -1; dz <= 1; dz++){
          if( (dz==-1 && pos[2]<1e-8) || (dz==1 && pos[2]>1-1e-8) ) continue;
          int weight = 1 << (abs(dx) + abs(dy) + abs(dz));
          system_rhs[i] += src[cf + dx*pw2 + dy*pw + dz] * 1./weight * 0.125;
        }
      }
    }
  }
    ...
    for(int dx = -1; dx <= 1; dx++){
      ...
      for(int dy = -1; dy <= 1; dy++){
        ...
        for(int dz = -1; dz <= 1; dz++){
          if( (dz==-1 && pos[2]<1e-8) || (dz==1 && pos[2]>1-1e-8) ) continue;
          int weight = 1 << (abs(dx) + abs(dy) + abs(dz));
          dst[cf + dx*pw2 + dy*pw + dz] += 1./weight * solution[i];
        }
      }
    }
  }
...
class RHS : public Function<3>{
public:
  double value(const Point<3> & p, const unsigned int component = 0) const override{
    (void)component;
    return 30. * M_PI * M_PI * sin(M_PI*p[0]) * sin(M_PI*p[1]) * sin(M_PI*p[2]);
  };
};


class Solution : public Function<3>{
public:
  double value(const Point<3> & p, const unsigned int component = 0) const override{
    (void)component;
    return 10. * sin(M_PI*p[0]) * sin(M_PI*p[1]) * sin(M_PI*p[2]);
  };
  ...
  gradient(const Point<3> & p, const unsigned int component = 0) const override{
    (void)component;
    Tensor<1, 3> grad;
    grad[0] = 10. * M_PI * cos(M_PI*p[0]) * sin(M_PI*p[1]) * sin(M_PI*p[2]);
    grad[1] = 10. * M_PI * sin(M_PI*p[0]) * cos(M_PI*p[1]) * sin(M_PI*p[2]);
    grad[2] = 10. * M_PI * sin(M_PI*p[0]) * sin(M_PI*p[1]) * cos(M_PI*p[2]);
    return grad;
  };
};
...
void Elliptic::setup_system(){
  ...
  triangulation.refine_global(level);
  dof_handler.distribute_dofs(fe);
  std::cout << "DoF number: " << dof_handler.n_dofs() << std::endl;
  dof_location_map = DoFTools::map_dofs_to_support_points(MappingFE<3>(fe), dof_handler);
...
void Elliptic::output_results() const
{
  ...
  VectorTools::integrate_difference(MappingFE<3>(fe),
                                    dof_handler,
                                    solution,
                                    Solution(),
                                    difference_per_cell,
  ...
  const QIterated<3> q_iterated(q_trapez, fe.degree*2+1);
  VectorTools::integrate_difference(MappingFE<3>(fe),
                                    dof_handler,
                                    solution,
                                    Solution(),
