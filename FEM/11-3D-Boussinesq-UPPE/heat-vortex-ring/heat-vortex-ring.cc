...
template <int dim>
class InitialTermTemperature : public Function<dim>
{
public:
...
template <int dim>
double InitialTermTemperature<dim>::value(const Point<dim> & p,
                                  const unsigned int component) const
{
  ...
  double theta = acos(dp[0] / A);
  if(dp[1] < 0) theta = 2*M_PI - theta;
...
template <int dim>
class InitialTermU1 : public Function<dim>
{
public:
  virtual double value(const Point<dim>  &p,
                        const unsigned int component = 0) const override;
...
template <int dim>
double InitialTermU1<dim>::value(const Point<dim> & p,
                                  const unsigned int component) const
{
  (void)component;
  Assert(component == 0, ExcIndexRange(component, 0, 1));
...
void Boussinesq<dim>::solve_pressure(Vector<double>& solution)
{
  constraints_pressure.condense(system_matrix_pressure, 
                                   system_rhs);
  SolverControl            solver_control(2000, 1e-7);
  SolverCG<Vector<double>> solver(solver_control);

  MGTransferPrebuilt<Vector<double>> mg_transfer(mg_constrained_dofs);
  mg_transfer.build(dof_handler);

  SolverControl coarse_solver_control(5000, 1e-8, false, false);
