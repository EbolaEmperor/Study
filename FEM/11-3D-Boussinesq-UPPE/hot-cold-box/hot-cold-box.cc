//----------------------------------------------------------
// This code is for cold-box test
// 
...
void Boussinesq<dim>::make_mesh(){
  Triangulation<dim> top_box, box, bottom_box, tmp1, tmp2, tmp3;
  GridGenerator::subdivided_hyper_rectangle(top_box,
                                            std::vector<unsigned int>({1U, 1U, 5U}),
                                            Point<dim>(5./11, 5./11, 1.),
                                            Point<dim>(6./11, 6./11, 16./11));
  GridGenerator::subdivided_hyper_rectangle(box,
                                            std::vector<unsigned int>({11U, 11U, 11U}),
                                            Point<dim>(0., 0., 0.),
                                            Point<dim>(1., 1., 1.));
  GridGenerator::subdivided_hyper_rectangle(bottom_box,
                                            std::vector<unsigned int>({1U, 1U, 5U}),
                                            Point<dim>(5./11, 5./11, -5./11),
                                            Point<dim>(6./11, 6./11, 0.0));
  GridGenerator::flatten_triangulation(top_box, tmp1);
  GridGenerator::flatten_triangulation(bottom_box, tmp2);
  GridGenerator::merge_triangulations(tmp1, box, tmp3);
  GridGenerator::merge_triangulations(tmp3, bottom_box, triangulation);
  ...
  for(const auto& cell : triangulation.active_cell_iterators())
    for(const auto& face : cell->face_iterators())
      if (face->at_boundary())
      {
        if (face->center()[2] > 1.0001)
          face->set_all_boundary_ids(2);
        else if (face->center()[2] < -0.0001)
      ...
      for (const unsigned int q_index : fe_values.quadrature_point_indices())
      {
        ...
        for (const unsigned int i : fe_values.dof_indices())
        {
          cell_rhs(i)    += weight * fe_values.shape_value(i, q_index) * 
                          ...
                          + u3_q_point[q_index] * grad_T_q_point[q_index][2]);
        }
      }
...
void Boussinesq<dim>::run(){
  ...
  L_u3_stage1.reinit(solution_u1.size());
  ...
  while(time <= end_time){
    // ------------------------------first stage-------------------------------------
    // update p^n
    update_pressure(prev_solution_u1, prev_solution_u2, prev_solution_u3, prev_temperature);
    output_result();
    ...
    system_rhs.add(-0.5*time_step, tmp);
    L_u3_stage1 = convection_u3;
    L_u3_stage1 *= -1.;
    L_u3_stage1.add(-1., grad_pressure_u3);
    setup_forcing(prev_temperature);
    L_u3_stage1.add(1., forcing);
    system_rhs.add(time_step, L_u3_stage1);
    
    solve_velocity(middle_solution_u3);
    ...
    system_rhs.add(-0.5*time_step, tmp);
    setup_u_grad_T(prev_solution_u1, prev_solution_u2, prev_solution_u3, prev_temperature);
    ...
    // update p*
    update_pressure(middle_solution_u1, middle_solution_u2, middle_solution_u3, middle_temperature);
    ...
    system_rhs.add(-0.5*time_step, tmp);
       // L_pre + L*
    system_rhs.add(0.5*time_step, L_u3_stage1);
    ...
    setup_u_grad_T(middle_solution_u1, middle_solution_u2, middle_solution_u3, middle_temperature);
...
int main(int argc, const char *argv[]){
  ...
  Boussinesq<3> boussinesq(level, end_time);
  boussinesq.run();
  return 0;
}